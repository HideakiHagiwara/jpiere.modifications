...
*/
	...
	public boolean allocateIt()
	{
		...
		for (int i = 0; i < pAllocs.length; i++)
		{
			...
			if (isReceipt())
				aLine = new MAllocationLine (alloc, pa.getAmount().compareTo(pa.getInvoiceAmt()) > 0 ? pa.getInvoiceAmt() : pa.getAmount(), //JPIERE-0214
					pa.getDiscountAmt(), pa.getWriteOffAmt(), pa.getOverUnderAmt());
			else
				aLine = new MAllocationLine (alloc, pa.getAmount().compareTo(pa.getInvoiceAmt()) > 0 ? pa.getInvoiceAmt().negate() : pa.getAmount().negate(),//JPIERE-0214 
